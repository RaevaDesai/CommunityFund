            ...
            Section {
                ...
                Toggle("I donated", isOn: $hasDonated)
                    .onChange(of: hasDonated) { newValue in
                        guard let fundraiserId = fundraiser.id else {
                            print("❌ Fundraiser ID is nil – donation not saved")
                            return
                        }

                        if newValue {
                            authVM.addDonation(fundraiserId: fundraiserId)
                        } else {
                            authVM.removeDonation(fundraiserId: fundraiserId)
                        }

                        UserDefaults.standard.set(newValue, forKey: "donated_\(fundraiserId)")
                    }
